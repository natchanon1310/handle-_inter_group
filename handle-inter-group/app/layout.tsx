...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="dark scroll-smooth" suppressHydrationWarning>
      ...
      <body className="bg-[#0d0f12] text-zinc-100 antialiased overflow-x-hidden selection:bg-orange-500 selection:text-white">
        <Navbar />
        ...
        {/* Footer Area - แก้ไข mt-20 เป็น mt-0 เพื่อลบช่องว่างสีขาวเหนือ Footer */}
        <footer 
          ref={footerRef}
          style={{
            opacity: isFooterVisible ? 1 : 0,
            transform: isFooterVisible ? "translateY(0) scale(1)" : "translateY(40px) scale(0.98)",
          }}
          className="bg-[#12151b] text-zinc-400 text-xs py-20 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) transform-gpu mt-0 border-t border-white/5"
        >
              ...
              <div className="w-full h-52 rounded-2xl overflow-hidden border border-white/10 relative group shadow-2xl bg-zinc-900">
                ...
                <div className="absolute bottom-3 left-3 right-3 pointer-events-none transition-all duration-300 group-hover:opacity-0">
                  <span className="bg-[#0d0f12]/90 backdrop-blur-md text-zinc-200 text-[10px] font-medium px-3 py-2 rounded-full block text-center border border-white/10 shadow-lg">
                    <i className="fa-solid fa-map-location-dot text-orange-500 mr-1.5"></i>{fText.mapBtn}
                  </span>
                </div>
              </div>
            </div>
